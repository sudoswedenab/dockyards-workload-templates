...
_secretList: [
	for remoteWriteConfig in #workload.spec.input.remoteWrite {
		...
		data: {
			"tls.crt":  '\(base64.Encode(null, remoteWriteConfig.mTLS.cert))'
			"password": '\(base64.Encode(null, remoteWriteConfig.mTLS.key))'
		}
	},
]
...
_values: apiextensionsv1.#JSON & {
	extraVolumes: [
		{
			name: "promtail-client-cert"
			secret:
				secretName: "promtail-client-cert"
		},
	]
	extraVolumeMounts: [
		{
			...
			if #workload.spec.input.remoteWrite != _|_ {
				remoteWrite: [
					for remoteWriteConfig in #workload.spec.input.remoteWrite {
						...
						tenant_id: remoteWriteConfig.mTLS.tenantID
						tls_config: {
							cert_file:            remoteWriteConfig.mTLS.cert
							key_file:             remoteWriteConfig.mTLS.key
